...
const MediaUploadComponent = ({ formData, setFormData }) => {
  ...
  useEffect(() => {
    ...
    newSocket.on("connect", () => {
      setIsConnected(true);
      console.log("Socket connected for uploads");
    });
  ...
  // Listen for mobile uploads via WebSocket
  useEffect(() => {
    if (socket && uploadToken) {
      ...
      const handleMobileUpload = (imageData) => {
        ...
        // Add the uploaded image to form data (TEMP URL for now)
        const newImage = {
          file: null, // No file object since it's from mobile
          preview: imageData.url, // Temp S3 URL
          name: imageData.name,
          fromMobile: true,
          mobileUploadId: imageData.id,
          uploadedAt: imageData.uploadedAt,
          isTemp: true, // Mark as temporary
          tempUrl: imageData.url, // Store temp URL for later processing
        };
  ...
  }, [socket, uploadToken, setFormData]);
  ...
  const handleGenerateQRCode = async () => {
    try {
      ...
      if (qrCode) {
        ...
        const serverUrl = URL || "http://localhost:5000";
        await fetch(`${serverUrl}/api/products/register-upload-token`, {
          method: "POST",
          headers: {
